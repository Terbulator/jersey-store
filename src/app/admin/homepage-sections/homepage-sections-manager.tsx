...
export function HomepageSectionsManager({ items }: { items: HomepageSectionRow[] }) {
  ...
  return (
    <div className="overflow-hidden rounded-md border border-[#292929] bg-[#111111]">
      <ul className="divide-y divide-[#292929]">
        {items.map((s, i) => (
          <li key={s.id} className="flex items-center gap-3 px-4 py-3 hover:bg-[#171717]">
            <div className="flex flex-col">
              <button
                onClick={() => act(s.id, { sort_order: s.sort_order - 1 })}
                disabled={i === 0 || busy === s.id}
                aria-label="Move up"
                className="rounded p-0.5 text-[#666666] hover:text-[#EFECE6] disabled:opacity-30"
              >
                <ChevronUp className="h-3.5 w-3.5" />
              </button>
              <button
                onClick={() => act(s.id, { sort_order: s.sort_order + 1 })}
                disabled={i === items.length - 1 || busy === s.id}
            ...
            <button
              onClick={() => act(s.id, { enabled: !s.enabled })}
              disabled={busy === s.id}
              className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-semibold uppercase transition-colors disabled:opacity-50 ${
                s.enabled ? 'bg-[#4ADE80]/15 text-[#4ADE80]' : 'border border-[#292929] text-[#666666]'
              }`}
            >
