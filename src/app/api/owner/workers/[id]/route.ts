import { NextRequest, NextResponse } from 'next/server';
import { prisma } from '@/lib/prisma';
import { getOwnerUser } from '@/lib/owner-guard';
import { logAudit } from '@/lib/audit';

export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  const guarded = await getOwnerUser();
  if (!guarded.ok) return guarded.error;
  const { user } = guarded;

  const existing = await prisma.worker.findFirst({ where: { id: params.id, ownerId: user.id } });
  if (!existing) return NextResponse.json({ error: 'Worker not found' }, { status: 404 });

  const body = await req.json();
  const data: Record<string, unknown> = {};
  if (body.name !== undefined) data.name = body.name;
  if (body.phone !== undefined) data.phone = body.phone ?? null;
  if (body.status !== undefined) data.status = body.status;
  if (Object.keys(data).length === 0) return NextResponse.json({ error: 'Nothing to update' }, { status: 400 });

  const updated = await prisma.worker.update({ where: { id: params.id }, data });

  await logAudit({
    actorId: user.id,
    actorEmail: user.email,
    actorRole: user.role,
    action: body.status === 'INACTIVE' ? 'worker.deactivate' : 'worker.update',
    resource: 'worker',
    resourceId: params.id,
    oldValues: { name: existing.name, status: existing.status },
    newValues: { name: updated.name, status: updated.status },
  });

  return NextResponse.json({ worker: updated });
}

export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  const guarded = await getOwnerUser();
  if (!guarded.ok) return guarded.error;
  const { user } = guarded;

  const existing = await prisma.worker.findFirst({ where: { id: params.id, ownerId: user.id } });
  if (!existing) return NextResponse.json({ error: 'Worker not found' }, { status: 404 });

  await prisma.worker.delete({ where: { id: params.id } });

  await logAudit({
    actorId: user.id,
    actorEmail: user.email,
    actorRole: user.role,
    action: 'worker.delete',
    resource: 'worker',
    resourceId: params.id,
    oldValues: { name: existing.name },
  });

  return NextResponse.json({ ok: true });
}
