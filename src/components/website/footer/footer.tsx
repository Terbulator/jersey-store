...
export function Footer({ navItems, footer }: { navItems: NavItem[]; footer?: FooterSettings }) {
  ...
  return (
    <footer className="relative bg-black text-off-white" style={F.bg ? { background: F.bg } : undefined}>
      <nav
        aria-label="Footer"
        className="mx-auto grid w-full max-w-[1400px] grid-cols-2 gap-x-8 gap-y-12 px-6 pt-16 sm:grid-cols-4 sm:px-8 sm:pt-20 lg:gap-x-16 lg:px-12 lg:pt-24"
      >
        {columns.map((col) => (
          <div key={col.title}>
      ...
      <div className="h-[16vh] lg:h-[12vh]" aria-hidden="true" />
      ...
      {F.show_wordmark && (
        <div className="relative w-full min-h-[22vw] sm:min-h-[26vw] lg:min-h-[22vw] flex items-end">
          <div className="w-full px-6 sm:px-8 lg:px-12 pb-6 sm:pb-8 lg:pb-10">
            <FooterWordmark />
          </div>
        </div>
      )}
