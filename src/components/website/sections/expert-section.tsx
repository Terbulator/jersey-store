...
const DEFAULT_IMAGE = 'https://images.unsplash.com/photo-1523381210434-271e8be1f52b?w=1200&q=80';
const DEFAULT_ARGS = [
...
interface XSSettings { heading?: string; subheading?: string; arguments?: { value: string; label: string }[]; image_url?: string }

export function ExpertSection({ settings }: { settings?: XSSettings | null }) {
  const heading = settings?.heading ?? 'History';
  const sub = settings?.subheading ?? "Won't Skip Us.";
  const args = settings?.arguments ?? DEFAULT_ARGS;
