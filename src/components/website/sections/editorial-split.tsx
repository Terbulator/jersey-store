    ...
    <section className="bg-black py-20 sm:py-32 overflow-hidden">
      <div className="mx-auto max-w-[1400px] px-6 sm:px-8 lg:px-12 grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
        <motion.div initial={{ opacity: 0, scale: 1.05 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 1, ease: EASE_PREMIUM }} className="relative aspect-[4/5] overflow-hidden">
          <img src={image} alt="Jersey detail" className="w-full h-full object-cover" />
        ...
        <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.8, ease: EASE_PREMIUM }}>
          ...
          <h2 className="headline text-4xl sm:text-5xl lg:text-6xl text-off-white mt-4 leading-[1.05]">{heading}<br /><em className="text-off-white/50 not-italic">{sub}</em></h2>
          <p className="mt-6 text-base sm:text-lg text-off-white/60 max-w-md leading-relaxed">Every piece is cut from premium fabrics and checked stitch-by-stitch before it makes the drop list. No knock-offs. No shortcuts.</p>
          ...
          <Link href={ctaUrl} className="btn-pill btn-pill-outline mt-12">{cta}</Link>
        </motion.div>
      </div>
    </section>
  );
