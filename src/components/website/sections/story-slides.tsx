...
const FALLBACK_FOOTBALL = 'https://images.unsplash.com/photo-1485291723934-4b48f2736edd?w=1400&q=80';
const FALLBACK_CRICKET = 'https://images.unsplash.com/photo-1531415074968-036ba1b575da?w=1400&q=80';
...
const DEFAULT_SLIDES: StorySlide[] = [
  { id: 'football', code: '01', label: 'Football', headline: 'The game starts here.', sub: '', cta: 'Shop Football', href: ROUTES.FOOTBALL, image_url: FALLBACK_FOOTBALL },
  { id: 'cricket', code: '02', label: 'Cricket', headline: 'Play different.', sub: '', cta: 'Shop Cricket', href: ROUTES.CRICKET, image_url: FALLBACK_CRICKET },
  { id: 'player', code: '03', label: 'Player Version', headline: 'Built for the game.', sub: 'Player Version 25/26', cta: 'Explore Player Version', href: ROUTES.SHOP, image_url: 'https://images.unsplash.com/photo-1575361204480-aadea25e6e68?w=1400&q=80' },
  { id: 'master', code: '04', label: 'Master Edition', headline: 'Master the details.', sub: '', cta: 'Explore Master Edition', href: ROUTES.SHOP, image_url: 'https://images.unsplash.com/photo-1585591189603-d914a73ad1e1?w=1400&q=80' },
  { id: 'newdrop', code: '05', label: 'New Drop', headline: 'The new drop.', sub: '2026 Collection', cta: 'Shop New Drop', href: ROUTES.NEW_ARRIVALS, image_url: 'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=1400&q=80' },
];

interface StorySettings { slides?: StorySlide[] }

export function StorySlides({ categories, settings }: { categories: Category[]; settings?: StorySettings | null }) {
  ...
  const slides: StorySlide[] = useMemo(() => {
    ...
    return [
      ...
      DEFAULT_SLIDES[2], DEFAULT_SLIDES[3], DEFAULT_SLIDES[4],
    ];
  }, [settings?.slides, foot?.image, cric?.image]);

  const clamp = (n: number, min: number, max: number) => Math.min(Math.max(n, min), max);
  const [active, setActive] = useState(0);

  useEffect(() => {
    return scrollYProgress.on('change', (v) => setActive(clamp(Math.floor(v * slides.length), 0, slides.length - 1)));
      ...
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <div className="absolute bottom-0 right-0 z-10 pb-6 pr-6 sm:pb-8 sm:pr-8 lg:pb-8 lg:pr-12 flex items-center gap-4">
          <div className="w-24 sm:w-32 h-px bg-[rgba(239,236,230,0.25)] relative overflow-hidden">
            <motion.div className="absolute inset-y-0 left-0 bg-[#B3001B]" animate={{ width: `${((active + 1) / slides.length) * 100}%` }} transition={{ duration: 0.4, ease: EASE_PREMIUM }} />
          </div>
          <p className="font-mono-meta text-[11px] tracking-[0.2em] text-[#EFECE6]"><span className="text-[#B3001B]">{String(active + 1).padStart(2, '0')}</span> / {String(slides.length).padStart(2, '0')}</p>
        </div>
      </div>
    </section>
  );
