      ...
      <div className="mx-auto max-w-[1400px] px-6 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.8, ease: EASE_PREMIUM }}>
            ...
            <h2 data-cms="heading" data-sec-h style={{ color: 'var(--sec-heading, var(--th-text-inverse))' }} className="headline text-4xl sm:text-5xl lg:text-6xl text-off-white mt-4 leading-[1.05]">{heading}<br /><em style={{ color: 'var(--sec-accent, var(--th-brand-primary))' }} className="text-red not-italic">{sub}.</em></h2>
            <p style={{ color: 'var(--sec-body, var(--th-text-inverse-soft))' }} className="mt-6 text-base sm:text-lg text-off-white/60 max-w-md leading-relaxed">Stack jerseys, drop the price. Every bundle ships free, gift-wrapped, ready to flex.</p>
            <div data-cms="pricing" className="mt-10 space-y-3">
              {pricing.map((tier) => (
