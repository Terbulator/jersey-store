      ...
      <div className="mx-auto max-w-[720px] px-6 sm:px-8 text-center relative overflow-hidden">
        ...
        >
          ...
          <h2 className="headline text-4xl sm:text-5xl text-off-white mt-4 leading-[1.05]">
            STAY IN
            <br />
            <em className="text-red not-italic">THE CULTURE.</em>
          </h2>
          <p className="mt-6 text-base text-off-white/60 leading-relaxed max-w-md mx-auto">
          ...
          <form onSubmit={handleSubmit} className="mt-10 flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            ...
            <button
              type="submit"
              disabled={status === 'loading' || status === 'success'}
              className="btn-pill btn-pill-solid whitespace-nowrap"
            >
              {status === 'loading'
                ? 'Subscribing…'
                : status === 'success'
          ...
          <p
            className={`mt-4 text-sm ${status === 'error' ? 'text-red' : 'text-off-white/50'}`}
            role={status === 'error' ? 'alert' : 'status'}
          >
            {status === 'error' ? 'Please enter a valid email address.' : ''}
            {status === 'success' ? 'Welcome to HEADERR. Check your inbox.' : ''}
          </p>

          <p className="mt-6 font-mono-meta text-[9px] text-off-white/40">
            Earliest access. Exclusive drops. The usual.
          </p>
        </motion.div>
      </div>
    </section>
